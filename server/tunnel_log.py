...
# events survive a power cut and can be read back off the device with
# `mpremote fs cat :tunnel.log`.
#
...
#
# Timestamps come from the device clock, which nothing syncs — with no NTP it
# starts at the board's power-on epoch, so a stamp reads as uptime rather than
# wall time. It is still what you want: it orders events and shows the gaps.
#
...
def _stamp():
    t = time.localtime()
    return "%02d-%02d %02d:%02d:%02d" % (t[1], t[2], t[3], t[4], t[5])
